public class LeasingCost {
	...
	public static Vehicle createVehicle(String description){
        ...
        if(description.contains("charger:")){
            String charger=description.substring(description.indexOf("charger:")+8);
            charger= charger.substring(0,charger.indexOf(";"));
            information = new Fuel (usagedouble, Integer.parseInt(charger));
        } else {
            information = new Fuel (usagedouble);
        }
        return new Vehicle (name, information , new Lease(dueAtSigningdouble,leaseLengthint,monthlyCostdouble,mileageAllowanceint));
	}
	...
	public static void computeCO2EmissionsAndCost( Vehicle[] vehicles, double gasPrice, double electricityPrice ){
        ...
        vehicles[i].setCO2Emission(computeCO2(vehicles[i].getLease ().getLeaseLength(), vehicles[i].getFuel().getUsage(), vehicles[i].getLease ().getMileageAllowance (), co2));
